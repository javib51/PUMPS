...
// Global queuing stub
__global__ void gpu_global_queuing_kernel (int *nodePtrs, int *nodeNeighbors, int *nodeVisited,
  ...
  for(int nbrIdx = nodePtrs[node]; nbrIdx <nodePtrs[node + 1]; nbrIdx++){
    ...
    if (nodeVisited[neighbor]==0){
      ...
      int nextId = atomicAdd(numNextLevelNodes,1);
      nextLevelNodes[nextId] = neighbor;
    }
  }
}
...
__global__ void gpu_block_queuing_kernel(
  ...
  __shared__ int nextLevelNodes_private[BQ_CAPACITY];
  __shared__ int numNextLevelNodes_private;
	
  if (threadIdx.x == 0)  numNextLevelNodes_private = 0;
  __syncthreads();
  ...
  for(int nbrIdx = nodePtrs[node]; nbrIdx < nodePtrs[node + 1]; nbrIdx++){
    ...
    if (nodeVisited[neighbor] == 0){
      // Mark it and add it to the queue
      nodeVisited[neighbor] = 1;
      int nextId = atomicAdd(&numNextLevelNodes_private,1);
      nextLevelNodes_private[nextId] = neighbor;
    }
  }
  __syncthreads();

  if (threadIdx.x==0) {
    int nextId = atomicAdd(numNextLevelNodes,numNextLevelNodes_private);
	  
	  for( int i = 0; i < numNextLevelNodes_private; i++){
		   nextLevelNodes[nextId+i] = nextLevelNodes_private[i];
      }
  }
}
