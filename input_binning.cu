...
__global__ void gpu_normal_kernel(float *in_val, float *in_pos, float *out,
	...
	while( inIdx < num_in ) {
    	...
    	for (int outIdx = 0; outIdx < grid_size; ++outIdx) {
      		...
      		atomicAdd(&out[outIdx], in_val2 / (dist * dist));
  		}	
		inIdx+=next;
	}
}
...
__global__ void histogram(float *in_pos, int *bin_counts, int num_in,
    ...
    int inIdx = threadIdx.x + blockIdx.x * blockDim.x;
	int next = blockIdx.x * blockDim.x;
...
__global__ void scan(int *bin_counts, int *bin_ptrs) {
  ...
  int binIdx = threadIdx.x + blockIdx.x * blockDim.x;
  int next = blockIdx.x * blockDim.x;
	
	while (binIdx < NUM_BINS) {
  
    	bin_ptrs[binIdx + 1] =  bin_counts[binIdx];
    	bin_ptrs[binIdx + 2] =  bin_counts[binIdx + 1];
		binIdx += next;
	}
    __syncthreads();
    ...
    if (!threadIdx.x) {
      for(int i = 0; i < NUM_BINS; i++) {
		bin_ptrs[binIdx + 1] += bin_ptrs[binIdx];
      }
